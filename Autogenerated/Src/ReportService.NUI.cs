namespace Terrasoft.Configuration.ReportService
{
	...
	public class WordFieldUtility {
		...
		public static void FillWordFieldsInElement(Dictionary<string, DataValue> values, OpenXmlElement element,
			...
			foreach (KeyValuePair<string, PictureData> pictureData in picturesData) {
				...
				bool isPrintCompressionState = pictureData.Value.Picture.BlipFill.Blip.CompressionState != null &&
					pictureData.Value.Picture.BlipFill.Blip.CompressionState.InnerText == "print";
				if (imageStream != null) {
				...
				} else if (element is TableRow && !isPrintCompressionState) {
	...
	public class OpenXmlUtility: BaseOpenXmlUtility
	{
		...
		internal WordprocessingDocument GetWordReport(WordprocessingDocument docx, Dictionary<string, DataValue> values) {
			...
			foreach (HeaderPart hpart in docx.MainDocumentPart.HeaderParts) {
				ConvertFieldCodes(hpart.Header, false, new Dictionary<string, DataValue>());
				WordFieldUtility.FillWordFieldsInElement(values, hpart.Header, docx.MainDocumentPart);
				hpart.Header.Save();
			}
			foreach (FooterPart fpart in docx.MainDocumentPart.FooterParts) {
				ConvertFieldCodes(fpart.Footer, false, new Dictionary<string, DataValue>());
				WordFieldUtility.FillWordFieldsInElement(values, fpart.Footer, docx.MainDocumentPart);
				fpart.Footer.Save();
			}
			docx.MainDocumentPart.Document.Save();
			return docx;
	...
	public class AdditionalMacrosUtility : BaseOpenXmlUtility
	{
		...
		private string GetAdditionalMacrosPath(Guid schemaUId, string referenceColumnCaption, Guid columnUId) {
			var entitySchemaManager = userConnection.EntitySchemaManager;
			var entitySchema = entitySchemaManager.GetInstanceByUId(schemaUId);
			var referenceColumn = FindByCaption(entitySchema, referenceColumnCaption);
			var column = entitySchema.Columns.FindByUId(columnUId);
			var metaPath = string.Concat("[", entitySchema.Name, ":", referenceColumn.Name, "].", column.Name);
			if (column.ReferenceSchemaUId != Guid.Empty) {
				...
				metaPath = string.Concat(metaPath, ".", referenceSchema.PrimaryDisplayColumn.Name);
			}
			return metaPath;
		}
		...
		private string GetNameMetaPath(string captionMetaPath) {
			Regex str = new Regex(@"(?<Object>[^(]+)\((?<ReferenceColumn>.+)\)\.(?<Column>.+)");
			Match m = str.Match(captionMetaPath);
			if (m.Success) {
				EntitySchema schema = FindSchemaByCaption(m.Groups["Object"].Value);
				if (schema == null) {
					return string.Empty;
				}
				EntitySchemaColumn referenceColumn = FindByCaption(schema, m.Groups["ReferenceColumn"].Value);
				EntitySchemaColumn column = FindByCaption(schema, m.Groups["Column"].Value);
				if (referenceColumn != null && column != null) {
					var metaPath = string.Concat("[", schema.Name, ":", referenceColumn.Name, "].", column.Name);
					if (column.ReferenceSchemaUId != Guid.Empty) {
						var entitySchemaManager = userConnection.EntitySchemaManager;
						var referenceSchema = entitySchemaManager.GetInstanceByUId(column.ReferenceSchemaUId);
						metaPath = string.Concat(metaPath, ".", referenceSchema.PrimaryDisplayColumn.Name);
			...
			foreach (var macrosDataItem in additionalMacrosesData) {
				...
				for (int i = 0; i < entityCollection.Count; i++) {
					for (int j = 0; j < rows.Length; j++) {
						...
						WordFieldUtility.FillWordFieldsInElement(values, tableRow, docx.MainDocumentPart);
