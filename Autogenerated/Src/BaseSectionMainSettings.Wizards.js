	...
	return {
		...
		methods: {
			...
			onValidate: function() {
				...
				if (isValid) {
					const tag = Terrasoft.Features.getIsEnabled("SectionPageWizard")
			...
			onSelectExistObjectButtonClick: function() {
				const config = {
					lookupConfig: {
						...
						searchData: this.get("SelectExistObjectSearchData")
					},
			...
			onSelectExistObjectButtonCallback: function(response, callback, scope) {
				...
				Terrasoft.chain(
					...
					function(next, entitySchema) {
						const uId = entitySchema.getUId();
						name = entitySchema.getName();
						caption = entitySchema.getDisplayValue();
						const config = {
							existEntityUId: uId,
							name: name,
							caption: caption,
						...
						this.createNewSectionEntities(next, config);
					},
					function() {
						this.$Code = name;
			...
			checkExistEntitySchema: function(schemaUId, packageUId, callback, scope) {
				...
				Terrasoft.chain(
					function(next) {
						Terrasoft.EntitySchemaManager.findPackageItem(schemaUId, packageUId, next, this);
					},
