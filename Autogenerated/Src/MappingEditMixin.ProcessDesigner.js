	...
	function(Ext, Terrasoft, ModalBox) {
		Ext.define("Terrasoft.configuration.mixins.MappingEditMixin", {
			...
			onParameterLabelClick: function(config) {
				...
				var renderTo = containerId + "-" + id + "-" + sandbox.id;
			...
			_getDisplayValue: function(parameter, element) {
				const displayValueMacros = Terrasoft.FormulaMacros.prepareProcessElementParameterDisplayValue(
					element.getDisplayValue(), parameter.getFullCaption());
			 ...
			 */
			_actualizeCollectionMapping: function(collectionParameterUId) {
				this.tryClearParentValue();
				this.clearNestedValues();
				this.fireEvent("collectionMappingReset", collectionParameterUId);
			},
			...
			tryClearParentValue: function() {
				...
				if (result) {
					...
					this.fireEvent("collectionMappingReset", parent.$UId);
				}
				return result;
			},
			...
			setMappingEditValue: function(parameterName, sourceValue, options, callback, scope) {
				...
				const mappingToNestedParameterAllowed = this.$ParentUId ||
					(!Terrasoft.isEmpty(this.$AllowedCollectionItemNestingLevel) && this.$AllowedCollectionItemNestingLevel > 0);
			...
			_processMappingToCollectionItem: function(config, callback, scope) {
				let parentParameter;
				if (this.$ParentUId) {
					parentParameter = this._findParameterInHierarchicalCollection(this.$Parameters, this.$ParentUId);
				} else {
					this.fireEvent("collectionMappingSet", config);
					parentParameter = config.parent;
				}
