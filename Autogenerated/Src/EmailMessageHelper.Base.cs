namespace Terrasoft.Configuration
{
	...
	public class EmailMessageHelper : IEmailMessageHelper
	{
		...
		/// </summary>
		private readonly Guid _fromRoleId = new Guid("6A6390C4-A6E1-DF11-971B-001D60E938C6");

		/// <summary>
		/// <see cref="ActivityParticipantRole"/> instance with code "To" unique identifier.
		/// </summary>
		private readonly Guid _toRoleId = new Guid("3A6893CE-A6E1-DF11-971B-001D60E938C6");
		...
		private void SendSocketMessage(JObject messageData, Guid sysAdminUnitId) {
			try {
				IMsgChannel channel = GetMessageChannel(sysAdminUnitId);
				if (channel == null) {
					return;
				}
		...
		protected Guid GetCurrentUserRole(Entity activity, Guid mailboxId) {
			if (mailboxId.IsEmpty() || !_userConnection.GetIsFeatureEnabled("MessageDirectionByMailbox")) {
		...
		protected virtual void SendUserMessage(Entity email) {
			...
			foreach (Guid sysAdminUnitId in GetUsersForNotificationList(email)) {
				SendSocketMessage(messageData, sysAdminUnitId);
			}
		}
		...
		protected virtual void AddFinishedSyncSession(string sessionId) {
			if (GetIsSessionExists(sessionId) || GetSessionMessagesCount(sessionId) == 0) {
				return;
			}
		...
		protected virtual IEnumerable<Guid> GetUsersForNotificationList(Entity email) {
			...
			if (_userConnection.GetIsFeatureEnabled("SharedMailboxes")) {
				Guid mailboxId = email.GetTypedColumnValue<Guid>("MailboxSyncSettings");
		...
		public virtual Entity CreateEmailMessage(Entity activity, Guid mailboxId, Dictionary<string, string> headers,
			...
			if (email.StoringState == StoringObjectState.New) {
				...
				if (save) {
					email.SetColumnValue("IsNeedProcess", activity.GetTypedColumnValue<bool>("IsNeedProcess"));
				}
		...
		public virtual void OnEmailMessageDataInserting(Entity emailMessageData) {
			...
			if (emailMessageData.StoringState == StoringObjectState.New) {
				bool activityIsNeedProcess = GetActivityIsNeedProcess(emailMessageData);
				emailMessageData.SetColumnValue("IsNeedProcess", activityIsNeedProcess);
			}
