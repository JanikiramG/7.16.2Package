namespace Terrasoft.Configuration.ML
{
	...
	public interface IMLDataLoader
	{
		...
		int LoadData(Select select, Action<string> onChunkLoaded);
	...
	public class MLDataLoader : IMLDataLoader
	{
		...
		private void OnTrainingChunkLoaded(DataTable dataTable, Action<string> onChunkLoaded) {
			string jsonString = JsonConvert.SerializeObject(dataTable, new IsoDateTimeConverter());
			onChunkLoaded(jsonString);
		}
		...
		/// </returns>
		public virtual int LoadData(Select select, Action<string> onChunkLoaded) {
