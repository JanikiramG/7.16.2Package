namespace Terrasoft.Configuration
{
	...
	using Terrasoft.Nui;
#if !NETSTANDARD2_0 // TODO CRM-44719
	...
	public class ConfigurationSectionHelper : ISectionHelper
	{
		...
		private Select ModuleStructureSelect {
			get {
		...
		private Select GetClientUnitSchemaNameSelect(string sourceAlias, string sourceColumnAlias) {
			var clientUnitSchemaNameSelect = (Select)new Select(SystemUserConnection)
				...
				.IsEqual(new QueryParameter(SysWorkspaceIdParameterName, _userConnection.Workspace.Id, GuidTypeName))
		...
		/// <summary>
		/// Gets module structure.
		/// </summary>
		/// <param name="userConnection">User connection.</param>
		/// <returns>Module structure.</returns>
		private Dictionary<string, string> GetModuleStructure(UserConnection userConnection) {
			_userConnection = userConnection;
			bool useMultilanguageData = !_userConnection.GetIsFeatureEnabled("DoNotUseMultilanguageData");
			...
			using (DBExecutor dbExecutor = _userConnection.EnsureDBConnection()) {
				using (IDataReader dataReader = ModuleStructureSelect.ExecuteReader(dbExecutor)) {
			...
			entitiesStructure = new Dictionary<Guid, EntityStructure>();
			using (DBExecutor dbExecutor = _userConnection.EnsureDBConnection()) {
...
#if !NETSTANDARD2_0 // TODO CRM-37504
		private void AddItemDescriptors<T>(StringBuilder descriptorsStringBuilder,
				IEnumerable<ISchemaManagerItem<T>> schemaManagerItems, CoreClientUnitSchemaDecorator decorator)
				where T : Schema, ISchemaManagerSchema<T> {
			foreach (ISchemaManagerItem<T> item in schemaManagerItems) {
				string descriptor = decorator.GetSchemaDescriptor(item);
		...
		protected string GetGoogleTagManagerScript(UserConnection userConnection) {
			...
			if (googleTagManagerScript.IsNotEmpty()) {
				string gtmScript = googleTagManagerScript.ToString().Replace("'", @"""").Replace("/script", @"\/script");
					...
					while (dataReader.Read()) {
						...
						if (!workplaces.ContainsKey(workplaceId)) {
							...
							workplaces.Add(workplaceId, string.Format(itemTmpl, workplaceId, workplaceNameValidate, position,
									workplaceLoaderId, loaderName, isPersonal));
						}
					}
				}
			}
		...
		public virtual string GetConfigurationScript(UserConnection userConnection) {
			...
			if (cachedScript.IsNullOrEmpty()) {
				Dictionary<string, string> config = GetModuleStructure(userConnection);
				var sb = new StringBuilder();
