namespace Terrasoft.Configuration.DesignService
{
	...
	public class DesignService
	{
...
#if !NETSTANDARD2_0
		private SqlScriptDescriptor CreateSqlScriptDescriptor(string code, DBEngineType dbEngineType, Package package) {
			return new SqlScriptDescriptor {
				UId = Guid.NewGuid(),
				Name = GetSqlScriptName(code, dbEngineType),
				InstallType = SysPackageSqlScriptInstallType.AfterPackage,
				DBEngineType = dbEngineType,
				Package = package
			};
		}

		private string GetModuleDiffScript(List<Guid> moduleIds, Package package, string code) {
			var packageElementUtilities = new PackageElementUtilities(UserConnection);
			packageElementUtilities.DeleteSqlScript(package.UId, GetSqlScriptName(code, DBEngineType.MSSql));
			packageElementUtilities.DeleteSqlScript(package.UId, GetSqlScriptName(code, DBEngineType.Oracle));
			var sbMSSQL = new StringBuilder();
			var sbOracle = new StringBuilder();
			const string deletSysModuleEditScriptTemplate = "delete from \"SysModuleEdit\" where \"TypeColumnValue\" in ({0}) ";
			const string separatorMSSQL = "GO";
			const string separatorOracle = "/";
			string recordIds = StringUtilities.Concat(moduleIds, ",", "'{0}'");
			sbMSSQL.AppendLine(separatorMSSQL);
			sbMSSQL.AppendFormat(deletSysModuleEditScriptTemplate, recordIds);
			sbMSSQL.AppendLine(separatorMSSQL);
			sbOracle.AppendLine(separatorOracle);
			sbOracle.AppendFormat(deletSysModuleEditScriptTemplate, recordIds);
			sbOracle.AppendLine(separatorOracle);
			SqlScriptDescriptor sqlScriptDescriptor = CreateSqlScriptDescriptor(code, DBEngineType.MSSql, package);
			packageElementUtilities.SaveSqlScript(sqlScriptDescriptor, sbMSSQL.ToString());
			sqlScriptDescriptor = CreateSqlScriptDescriptor(code, DBEngineType.Oracle, package);
			packageElementUtilities.SaveSqlScript(sqlScriptDescriptor, sbOracle.ToString());
			return UserConnection.DBEngine.DBEngineType == DBEngineType.Oracle ? sbOracle.ToString() : sbMSSQL.ToString();
		}

		private void UpdateSectionModuleDiffScript(string sectionCode, string sectionEntitySchemaName, Guid sectionTypeColumnUId,
				Package package, UserConnection userConnection) {
			EntitySchema sectionEntitySchema = userConnection.EntitySchemaManager.GetInstanceByName(sectionEntitySchemaName);
			string typeObjectName = string.Empty;
			if (sectionTypeColumnUId != Guid.Empty) {
				var sectionTypeColumn = sectionEntitySchema.Columns.GetByUId(sectionTypeColumnUId);
				typeObjectName = sectionTypeColumn.ReferenceSchema.Name;
			}
			var moduleIds = GetSysModuleEditDeleteList(sectionCode, typeObjectName);
			if (moduleIds.Count > 0) {
				GetModuleDiffScript(moduleIds, package, sectionCode);
			}
		}
#endif
...
#if !NETSTANDARD2_0
		private string GetCompilerErrorsMessage(CompilerErrorCollection compilerErrors) {
			StringBuilder errorMessage = new StringBuilder();
			foreach (CompilerError error in compilerErrors) {
				if (error.IsWarning) {
					string sourceWarningMessage = GetResource("SourceWarningMessage");
					errorMessage.AppendFormat(sourceWarningMessage, error.FileName, error.Line, 
						error.Column, error.ErrorNumber, error.ErrorText);
				} else {
					if (string.IsNullOrEmpty(error.FileName)) {
						string compilationErrorMessage = GetResource("CompilationErrorMessage");
						errorMessage.AppendFormat(compilationErrorMessage, error.ErrorNumber, error.ErrorText);
					} else {
						string sourceErrorMessage = GetResource("SourceErrorMessage");
						errorMessage.AppendFormat(sourceErrorMessage, error.FileName, error.Line, 
							error.Column, error.ErrorNumber, error.ErrorText);
					}
				}
			}
		...
		private CompilerError[] GetCompilerErrorsArray(CompilerErrorCollection compilerErrors) {
		...
		[WebInvoke(Method = "POST", RequestFormat = WebMessageFormat.Json, BodyStyle = WebMessageBodyStyle.Wrapped,
		ResponseFormat = WebMessageFormat.Json)]
		public Responce SaveScheme(string designData) {
			DesignData desirializedDesignData = ServiceStackTextHelper.Deserialize<DesignData>(designData);
			Responce response = CheckDesignData(desirializedDesignData);
			if (response.Success) {
				PackageUId = desirializedDesignData.CurrentPackageUId;
				SchemaManager schemaManager = desirializedDesignData.SchemaManager;
				var entitiesMetaInfo = schemaManager.Entity;
				var clientUnitsMetaInfo = schemaManager.ClientUnit;
				lock(_lockObject) {
					using (DBExecutor dbExecutor = SystemUserConnection.EnsureDBConnection()) {
						dbExecutor.StartTransaction();
						try {
							var ModifiedEntitySchemaUIds = new Collection<Guid>();
							foreach(var entityMetaInfo in entitiesMetaInfo) {
								Guid entitySchemaUId = entityMetaInfo.ApplyChanges();
								if (!entitySchemaUId.IsEmpty()) {
									ModifiedEntitySchemaUIds.Add(entitySchemaUId);
								}
							}
							Collection<Guid> ModifiedClientUnitSchemaUIds = new Collection<Guid>();
							foreach(var clientUnitMetaInfo in clientUnitsMetaInfo) {
								Guid clientUnitSchemaUId = clientUnitMetaInfo.ApplyChanges();
								ModifiedClientUnitSchemaUIds.Add(clientUnitSchemaUId);
							}
							bool hasErrors = false;
							bool needBuild = ModifiedEntitySchemaUIds.Count > 0;
							if (needBuild) {
								...
								CompilerErrorCollection compilerResult = workspaceBuilder.Build(appConnection.Workspace);
								hasErrors = compilerResult.HasErrors;
								if (hasErrors) {
									response.Success = false;
									response.ErrorMessage = GetCompilerErrorsMessage(compilerResult);
									response.CompilerErrors = GetCompilerErrorsArray(compilerResult);
								}
							} 
							if (hasErrors) {
								...
								this.ApplyEntitySchemeChanges(ModifiedEntitySchemaUIds);
							}
						} catch (Exception e) {
							dbExecutor.RollbackTransaction();
							response.Success = false;
		...
		[WebInvoke(Method = "POST", RequestFormat = WebMessageFormat.Json, BodyStyle = WebMessageBodyStyle.Wrapped,
		ResponseFormat = WebMessageFormat.Json)]
		public Responce SaveData(string designData) {
			DesignData desirializedDesignData = ServiceStackTextHelper.Deserialize<DesignData>(designData);
			Responce response = CheckDesignData(desirializedDesignData);
			if (response.Success) {
				lock (_lockObject) {
					using (DBExecutor dbExecutor = UserConnection.EnsureDBConnection()) {
						...
						try {
							PackageUId = desirializedDesignData.CurrentPackageUId;
							var packageId = WorkspaceUtilities.GetPackageIdByUId(PackageUId, UserConnection);
							var dataCollection = desirializedDesignData.Data;
							string[] schemaNameSequence = desirializedDesignData.SchemaNameSequence;
							for (int i = 0; i < schemaNameSequence.Length; i++) {
								string schemaName = schemaNameSequence[i];
								Data[] entitySchemaDataArray;
								var entitySchemaDataUIds = new List<object>();
								if (dataCollection.TryGetValue(schemaName, out entitySchemaDataArray)) {
									for (int j = 0; j < entitySchemaDataArray.Length; j++) {
										Guid dataUId = entitySchemaDataArray[j].ApplyChanges(schemaName);
										if (!dataUId.IsEmpty()) {
											entitySchemaDataUIds.Add(dataUId);
										}
									}
								}
								UpdateSchemaData(UserConnection, entitySchemaDataUIds, schemaName, packageId, desirializedDesignData.SectionCode);
							}
							UserProfile defaultGridSettings = desirializedDesignData.GridSettings;
							if (defaultGridSettings != null) {
								UserProfile userGridSettings = new UserProfile() {
									Key = defaultGridSettings.Key,
									Data = defaultGridSettings.Data,
									IsDef = false
								};
								userGridSettings = defaultGridSettings;
								UserProfileResourceHandler.SaveUserProfile(UserConnection, defaultGridSettings);
								UserProfileResourceHandler.SaveUserProfile(UserConnection, userGridSettings);
							}
							SaveDetailsProfile(desirializedDesignData.DetailGridSettings);
							dbExecutor.CommitTransaction();
							var packageInfo = GetPackagesInfo();
							string packageName = packageInfo[PackageUId];
							Package package = new Package(PackageUId, packageName);
							Guid sectionTypeColumnUId = desirializedDesignData.SectionTypeColumnUId;
							if (sectionTypeColumnUId != Guid.Empty) {
								UpdateSectionModuleDiffScript(desirializedDesignData.SectionCode, desirializedDesignData.SectionEntitySchemaName,
									sectionTypeColumnUId, package, UserConnection);
							}
						} catch (Exception e) {
							dbExecutor.RollbackTransaction();
							response.Success = false;
