namespace Terrasoft.Configuration
{
	...
	public class GCalendarSynchronizer : BaseGoogleSynchronizer
	{
		...
		private void GEventToActivity(GoogleActivity entry, ref Entity activity) {
			...
			TimeZoneInfo googleTimeZone = UserConnection.CurrentUser.TimeZone;
			if (!string.IsNullOrEmpty(entry.TimeZoneId)) {
				var timeZones = from zones in TimeZoneInfo.GetSystemTimeZones()
					where zones.Id == entry.TimeZoneId select zones;
				if (timeZones != null && timeZones.Count() == 1) {
					googleTimeZone = timeZones.FirstOrDefault();
				}
			}
		...
		private void ActivityParticipantsToGoogleEvent(GoogleActivity entry, Terrasoft.Configuration.Activity activity) {
		...
		private void ActivityToGEvent(Terrasoft.Configuration.Activity activity, ref GoogleActivity entry, bool isNewGActivity) {
			LogMessage("ActivityToGEvent: activity Id=" + activity.Id.ToString() + ", Title=" + activity.Title + ", From-To=" +
					   activity.StartDate + "-" + activity.DueDate, UserConnection, "GoogleCalendar");
			entry.Title = activity.Title;
			entry.StartDate =
				(DateTime)
					UserConnection.DataValueTypeManager.GetInstanceByName("DateTime")
						.GetValueForSave(UserConnection, activity.StartDate);
			entry.DueDate =
				(DateTime)
					UserConnection.DataValueTypeManager.GetInstanceByName("DateTime").GetValueForSave(UserConnection, activity.DueDate);
			if (isNewGActivity || !HasExtId(entry)) {
				SetGEventExtId(entry, activity);
			}
			LogMessage("ActivityToGEvent: entry ExtId=" + entry.ExtId + ", Title=" + entry.Title + ", From-To=" +
					   entry.StartDate + "-" + entry.DueDate, UserConnection, "GoogleCalendar");
