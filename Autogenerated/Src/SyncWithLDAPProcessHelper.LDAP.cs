	...
	public class SyncWithLDAPProcessHelper
	{
		...
		private void CheckUsersInGroup(LdapGroup group, Guid groupId, out DateTime? minModifiedDateOfNewUser) {
			...
			using (var ldapUtils = new LdapUtilities(_userConnection)) {
				usersCount = ldapUtils.InsertMembersOfGroupToTable(group);
			}
			var delete =
		...
		private void AddLicForUser(Entity user) {
			...
			if (_userConnection.GetIsFeatureEnabled("LDAPLicNotification")) {
		...
		private void CheckAllLDAPUsersInEverySyncGroup(out DateTime? minModifiedDateOfNewUser) {
			...
			if(_userConnection.GetIsFeatureEnabled("DeactivateExcludedLdapUsers")) {
				...
				_log.InfoFormat("{0} users from deleted groups were deactivated", deactivatedUsers);
			}
			var update =
				new Update(_userConnection, "SysAdminUnit")
		...
		private void DeactivateDeletedAdUsers() {
			if (!_userConnection.GetIsFeatureEnabled("DeactivateDeletedAdUsers")) {
				return;
			}
			try {
				using (var ldapUtils = new LdapUtilities(_userConnection)) {
					ldapUtils.InsertUsersIdsToTableByFilter();
					var update = 
						new Update(_userConnection, "SysAdminUnit")
							.Set("Active", Column.Parameter(false))
						.Where("LDAPEntryId")
					...
					_log.InfoFormat("{0} user(s) were deactivated in system, because they were deleted from AD.",
						deactivatedUsersCount);
				}
			} finally {
				var delete = new Delete(_userConnection).From("SysLDAPSynchUser");
				delete.Execute();
			}
		}

		private void DeactivateExcludedUsers() {
			...
			_log.InfoFormat("{0} users were deactivated", updateResult);
		}
		...
		public void SyncWithLDAP() {
			try {
				...
				DeactivateDeletedAdUsers();

				DeactivateExcludedUsers();
