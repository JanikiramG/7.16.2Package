namespace Terrasoft.Configuration.LDAP
{
	...
	public class LdapUtilities : IDisposable
	{
		...
		private int InsertUsersIdsToTableByFilter(string tableCode, string filter) {
			var result = 0;
		...
		internal int InsertUsersIdsToTableByFilter() {
			return InsertUsersIdsToTableByFilter("SysLDAPSynchUser", _ldapUsersFilter);
		}
		...
		public int InsertGroupsToTable() {
			return InsertGroupsToTable("SysLDAPSynchGroup");
		}
		...
		public int InsertUsersToTable() {
			return InsertUsersToTable("SysLDAPSynchUser");
		}

		public int InsertUsersToTable(string tableCode) {
			_log.DebugFormat("Inserting user data in table {0}.", tableCode);
			string ldapModifiedUsersFilter = GetUserFilterWithMinModifiedOnAttributeOrDate(null);
			int result = InsertUsersToTableByFilter(tableCode, ldapModifiedUsersFilter);
			_log.DebugFormat("{0} user records have been added to table {1}.", result, tableCode);
			return result;
		}
