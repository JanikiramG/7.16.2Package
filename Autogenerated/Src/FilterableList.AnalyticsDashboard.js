define("FilterableList", ["FilterableListResources", "AnalyticsDashboard", "css!FilterableList"],
	...
	Ext.define("Terrasoft.controls.FilterableList", {
		...
		/**
		 * The control configuration receive event name.
		 * @protected
		 */
		getControlConfigEventName: "getFilterableListConfig",

		/**
		 * The control configuration send event name.
		 * @protected
		 */
		setControlConfigEventName: "setFilterableListConfig",
		...
		tpl: [
			...
			'<ts-filterable-list id="{id}" data-qa="filterable-list">',
			'</ts-filterable-list>',
			'</div>',
			//jscs:enable
		...
		_initControlConfig: function() {
			...
			customEvent
				...
				.subscribe(function() {
					...
					this.mixins.customEvent.publish(this.setControlConfigEventName, config);
				}.bind(this));
		},
		 ...
		 * @protected
		 * @returns {Object} The instance of control configuration.
		 * {
		 * 		resources: {Object}
		 * }
		 */
