		...
		return {
			...
			methods: {
				...
				onAttachIconClick: function() {
					var type = this.get("Type");
					var typeid = type ? type.value : null;
					var config = this._getOpenPageConfig(EmailConstants.entitySchemaName, this.get("Id"), typeid);
					var valuePairs = config.valuePairs = config.valuePairs || [];
					valuePairs.push({
						name: "DefaultTabName",
				...
				_startEmailsInChainUpdate: function(currentColumnName, columnValue) {
					if (this.getIsFeatureDisabled("UpdateAllEmailsInChain") || this.isEmpty(this.$SendDate) ||
							this.isEmpty(columnValue)) {
						return;
					}
					this.showConfirmationDialog(this.get("Resources.Strings.ConfirmUpdateAllInChain"),
						function(returnCode) {
							if (returnCode === this.Terrasoft.MessageBoxButtons.YES.returnCode) {
								this._updateAllEmailsInChain(currentColumnName, columnValue);
				 ...
				 * @param {Object} columnValue Column value.
				 */
				_updateAllEmailsInChain: function(currentColumnName, columnValue) {
